...
const CorreosAutorizados: React.FC<CorreosAutorizadosProps> = ({ surveyId, isEditMode = false }) => {
  ...
  const cargarCorreos = async () => {
    try {
      ...
      const response = await fetch('https://egresados.it2id.cc/api/anonymous-emails', {
        headers: {
  ...
  const handleEliminarCorreo = async (correo: EmailData) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        const response = await fetch(`https://egresados.it2id.cc/api/anonymous-emails/${correo.id}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${userToken}`
          }
  ...
  const cargarInvitaciones = async () => {
    try {
      ...
      const response = await fetch(`https://egresados.it2id.cc/api/anonymous-invitations/survey/${surveyId}`, {
        headers: {
          'Authorization': `Bearer ${userToken}`
        }
      });
