...
const CrearEncuesta: React.FC = () => {
  ...
  const fetchSurveyData = useCallback(async () => {
    ...
    try {
      ...
      const surveyResponse = await fetch(`http://localhost:8000/surveys/${id}`, {
      ...
      // Fetch Questions
      const questionsResponse = await fetch(`http://localhost:8000/questions/survey/${id}`, {
        headers: { 'Authorization': `Bearer ${userToken}` }
      });
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!isEditMode) {
          ...
          const createSurveyResponse = await fetch('http://localhost:8000/surveys/', {
              method: 'POST',
              headers: {
          ...
          const surveyResponse = await fetch(`http://localhost:8000/surveys/${latestSurveyData.current.id}`, {
              method: 'PUT',
              headers: {
                  'Content-Type': 'application/json',
                  'Authorization': `Bearer ${userToken}`
      ...
      for (const question of questionsData) {
        ...
        if (question.id && question.id !== 0) { // If question has an ID, update it
          ...
          const questionResponse = await fetch(`http://localhost:8000/questions/${question.id}`, {
            method: 'PUT',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${userToken}`
          ...
          const createQuestionResponse = await fetch(`http://localhost:8000/questions/`, {
            method: 'POST',
            headers: {
              'Content-Type': 'application/json',
              'Authorization': `Bearer ${userToken}`
      ...
      for (const questionId of questionsToDelete) {
        ...
        const deleteQuestionResponse = await fetch(`http://localhost:8000/questions/${questionId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': `Bearer ${userToken}`
          }
  ...
  const enviarInvitaciones = async (surveyId: number): Promise<{exitosas: number, fallidas: number, total: number}> => {
    try {
      ...
      const emailsResponse = await fetch('http://localhost:8000/anonymous-emails', {
        headers: {
          'Authorization': `Bearer ${userToken}`
        }
      });
